...
class GeminiProcessor:
    def __init__(self, model_name, project, credential_file_name):
        import sys, os
        root_dir = os.path.abspath('../../')
        key_path = root_dir + '\\' + credential_file_name + '.json'
        self.credentials = Credentials.from_service_account_file(key_path)
        self.model = VertexAI(name=model_name, project= project, credentials= self.credentials)

    def generate_document_summary(self, documents: list, **args):
        chain_type = 'map_reducer' if len(documents) > 10 else 'stuff'
...
if __name__=='__main__':
    ...
    gemini_processor.generate_document_summary(documents=documents)
